    ...
    function tinhcongnhanvien($bean, $event, $arguments){
        ...
        if($duanBeans){
            ...
            foreach($nvccList as $nvcc){
                ...
                $query_tangca = "sgt_tangca.ma_nv = '".$nvcc->ma_nv."' AND sgt_tangca.sgt_duan_id_c = '".$duanid."' AND (sgt_tangca.ngay BETWEEN '".$tungay."' AND '".$denngay."')";
                $tongtangca_data = BeanFactory::getBean('sgt_tangca')->get_full_list('', $query_tangca);

                $tonggiotangca = 0;
                if($tongtangca_data){
                    foreach($tongtangca_data as $tangca){
                        $tonggiotangca += $tangca->tonggiotangca;
                    }
                }
                ...
                $ctnvBeans->tg_tangca = $tonggiotangca; 
                ...
                $tongtangca += $tonggiotangca; // Tổng hợp giờ tăng ca
            }
